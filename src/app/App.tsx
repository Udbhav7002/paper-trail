...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f1ea] text-gray-900 selection:bg-red-200 selection:text-red-900 overflow-x-hidden">
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-50 focus:bg-gray-900 focus:text-white focus:px-4 focus:py-2 focus:rounded focus:outline-none"
      >
      ...
      <header className="max-w-6xl mx-auto pt-8 px-4 sm:px-8 mb-8 border-b-4 border-gray-900 pb-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="font-mono text-gray-600 text-sm">
            A forensic analysis of digital life receipts.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div
            ...
            aria-live="polite"
            aria-label={`${discoveredCount} connections discovered`}
          >
            <Fingerprint size={14} aria-hidden="true" />
            <span className="font-bold">{discoveredCount}</span> discovered
          </div>
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    disabled={isDisabled}
                    className={`flex items-center gap-1.5 px-3 py-2 font-mono text-[11px] uppercase tracking-widest transition-colors min-h-[36px] ${
                      isActive
                    ...
                    title={isDisabled ? 'Click a receipt to view connections' : ''}
                  >
      ...
      <MotionConfig reducedMotion="user">
        <main id="main-content" className="px-4 sm:px-8">
          {error ? (
            <div className="py-20 text-center text-red-600 font-mono" role="alert">
      ...
      <footer className="max-w-6xl mx-auto px-4 sm:px-8 py-8 mt-12 border-t-2 border-dashed border-gray-300">
        <p className="font-mono text-xs text-gray-600 text-center">
          Paper Trail — React 19 · TypeScript · Tailwind CSS 4 · Zustand · Framer Motion · Built for WebRush
        </p>
      </footer>
    </div>
  );
